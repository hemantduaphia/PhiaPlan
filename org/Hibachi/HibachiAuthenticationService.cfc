component output="false" accessors="true" extends="HibachiService" {
	...
	public boolean function authenticateActionByAccount(required string action, required any account) {
		...
		if(getHibachiScope().getLoggedInFlag() && arguments.account.getAccountID() == getHibachiScope().getAccount().getAccountID()) {
			...
			if((listFindNocase(actionPermissions[ subsystemName ].sections[ sectionName ].anyLoginMethods, itemName) OR actionPermissions[ subsystemName ].sections[ sectionName ].anyLoginMethods == "*") && getHibachiScope().getLoggedInFlag()) {
				return true;
			}
