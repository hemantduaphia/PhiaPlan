...
function listingDisplayUpdate( tableID, data, afterRowID ) {
	...
	if( !listingUpdateHold( tableID, data, afterRowID ) ) {
		...
		data[ 'actionCallerAttributes' ] = {};
		
		jQuery.each(jQuery(tableHeadRowSelector).children(), function(ci, cv){
			
			if( jQuery(cv).hasClass('data') ) {
				if( jQuery(cv).data('actioncallerattributes') !== undefined ) {
					data[ 'actionCallerAttributes' ][ jQuery(cv).data('propertyidentifier') ] = jQuery(cv).data('actioncallerattributes');
				}
			}
		});
		
		// convert data[ 'actionCallerAttributes' ] to string so we can pass it in ajax request
		data[ 'actionCallerAttributes' ] = JSON.stringify(data[ 'actionCallerAttributes' ]) ;
